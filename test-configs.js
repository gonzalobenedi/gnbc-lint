...
function testConfig(configPath, description) {
  try {
    ...
    console.log(
      `  ✅ Configuration loads successfully and is valid flat config`
    );
    return true;
  } catch (error) {
    if (error.code === "MODULE_NOT_FOUND") {
      console.log(
        `  ⚠️  Skipped - requires optional peer dependency: ${
          error.message.match(/'(.+)'/)?.[1] || "unknown module"
        }`
      );
...
async function main() {
  ...
  console.log(
    "ℹ️  Note: Some configurations may be skipped due to missing peer dependencies"
  );
  ...
  for (const config of configs) {
    ...
    if (fs.existsSync(configPath)) {
      ...
      console.log(
        `⚠️  Configuration file ${config.path} not found, skipping...`
      );
    }
  }
  ...
  if (allPassed) {
    ...
    console.log(
      "💡 Install the appropriate peer dependencies to use specific configurations."
    );
    process.exit(0);
  } else {
